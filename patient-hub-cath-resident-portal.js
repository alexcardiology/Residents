...
let currentResidentName='',currentCode='',refreshTimer=null;
...
function renderRows(code,rows,residentName){
 const box=q('cathPortalRows');
 if(!box)return;
 if(!rows.length){
 ...
 <thead><tr style="background:#fff0e7">
   ...
   <th style="padding:13px">نوع التدخل</th>
   <th style="padding:13px">الاستشاري</th>
 ...
 <tbody>${rows.map(r=>{
   ...
   return `<tr style="border-top:1px solid #f2d7c8;background:${bg}">
     ...
     <td style="padding:13px">${esc(r.intervention_type||'—')}</td>
     <td style="padding:13px">${esc(r.consultant_name||'—')}</td>
     <td style="padding:13px">${esc(r.mobile||'—')}</td>
...
window.saveCathPortalReport=async(id,type,code)=>{
 ...
 const {error}=await sb.rpc('psh_cath_portal_save_item_report',{
   ...
   p_item_id:id,
   p_report_id:v,
   p_resident_name:currentResidentName,
   p_cd_received:cdReceived,
   p_outcome:outcome,
   p_postponed_reason:outcome==='postponed'?reason:null
 });
 if(error){alert(error.message);await fetchList(code,currentResidentName);return}
 await fetchList(code,currentResidentName);
};

async function logoutPortal(){stopAutoRefresh();const t=token();setToken('');currentResidentName='';if(t)try{await sb.rpc('psh_cath_portal_logout',{p_token:t})}catch{}location.reload()}
function showLogin(message=''){stopAutoRefresh();ensurePortalHost().style.display='none';q('appView')?.classList.add('hidden');if(q('appView'))q('appView').style.visibility='hidden';q('loginView')?.classList.remove('hidden');if(q('loginView'))q('loginView').style.visibility='visible';const email=q('email');if(email){email.type='text';email.placeholder='Email or username'}if(message&&q('loginMessage'))q('loginMessage').textContent=message}
...
document.addEventListener('keydown',e=>{
 ...
 const extra=q('cathExtraCaseModal');
 if(extra){extra.remove();return}
 const resident=q('cathResidentNameModal');
