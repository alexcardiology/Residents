...
function arDate(v){if(!v)return'';const d=new Date(v+'T12:00:00');const [y,m,day]=v.split('-');return `${AR_DAYS[d.getDay()]} ${day}/${m}/${y}`}
...
async function consultantConflict(code,date,consultant){
 ...
 const other=services.find(x=>x.code===otherCathCode(code));if(!other)return null;
 ...
 if(r.error){console.error('Consultant conflict check failed',r.error);return null}
 return r.data?.length?{site:siteLabel(other.code),count:r.data.length}:null;
}
function confirmConflict(conflict,consultant,date){
 if(!conflict)return true;
 return window.confirm(`تنبيه ⚠️\n\nالاستشاري ${consultant} لديه بالفعل قائمة قسطرة في ${conflict.site} يوم ${arDate(date)}.\n\nهل أنت متأكد من المتابعة والحجز في المكان الآخر في نفس اليوم؟`);
}
function installSingleConflictGuard(){
 const form=q('reservationForm');if(!form||form.dataset.conflictGuard==='1')return;
 form.dataset.conflictGuard='1';const original=form.onsubmit;
 form.onsubmit=async function(e){
   e.preventDefault();
   const code=q('rService')?.value?((services.find(x=>x.id===q('rService').value)?.code)||''):(typeof selectedCode!=='undefined'?selectedCode:'');
   const date=q('rDate')?.value||'',consultant=q('rConsultant')?.value?.trim()||'';
   if(isCathCode(code)){
