...
function doctorsFor(h){const seen=new Set(),out=[];h.forEach(r=>{const n=(r.consultant_name||'').trim();if(n&&!seen.has(norm(n))){seen.add(norm(n));out.push(n)}});return out}
async function enhancedSearchPatients(){
 ...
 if(!body)return;
 body.innerHTML=patients.map(p=>{const h=byPatient.get(p.id)||[];const hs=h.slice(0,6).map(r=>`<div style="margin:2px 0"><b>${esc(historyLabel(r))}</b>${r.scheduled_date?` <span style="opacity:.7">— ${esc(typeof fmt==='function'?fmt(r.scheduled_date):r.scheduled_date)}</span>`:''}</div>`).join('')||'<span style="opacity:.55">—</span>';const docs=doctorsFor(h);const ds=docs.length?docs.map(n=>`<div style="margin:2px 0">${esc(n)}</div>`).join(''):'<span style="opacity:.55">—</span>';return `<tr><td>${esc(p.arabic_name)}</td><td>${esc(p.mobile)}</td><td>${esc(p.national_id)}</td><td>${esc(p.year_of_birth)}</td><td style="min-width:190px">${ds}</td><td style="min-width:240px">${hs}</td><td><button class="link" onclick="patientModal('${p.id}')">${typeof lang!=='undefined'&&lang==='ar'?'فتح':'Open'}</button></td></tr>`}).join('')||`<tr><td colspan="7">${typeof lang!=='undefined'&&lang==='ar'?'لا يوجد مرضى مطابقون للبحث':'No patients found'}</td></tr>`;
}
...
function setup(){const box=$id('patientSearch'),btn=$id('searchBtn'),head=$id('patientRows')?.closest('table')?.querySelector('thead tr');if(!box||!btn)return;
 box.type='search';box.autocomplete='off';box.autocapitalize='off';box.spellcheck=false;box.name='patient_lookup_'+Date.now();box.value='';box.placeholder=(typeof lang!=='undefined'&&lang==='ar')?'الاسم / الهاتف / الرقم القومي / اسم الطبيب / الخدمة / نوع التدخل':'Name / mobile / National ID / doctor / service / intervention';
 if(head){let doctorHead=head.querySelector('[data-patient-doctor-head]');if(!doctorHead){doctorHead=document.createElement('th');doctorHead.dataset.patientDoctorHead='1';doctorHead.textContent=(typeof lang!=='undefined'&&lang==='ar')?'اسم الطبيب':'Doctor name';head.insertBefore(doctorHead,head.lastElementChild)}let histHead=head.querySelector('[data-patient-history-head]');if(!histHead){histHead=document.createElement('th');histHead.dataset.patientHistoryHead='1';histHead.textContent=(typeof lang!=='undefined'&&lang==='ar')?'الخدمة / نوع التدخل':'Service / intervention';head.insertBefore(histHead,head.lastElementChild)}}
