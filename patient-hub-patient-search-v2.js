...
function syncDrawerBrand(){const brand=document.querySelector('.drawerBrand');if(!brand)return;const strong=brand.querySelector('strong'),small=brand.querySelector('small');if(strong){strong.dataset.ar='مركز خدمات المرضى';if(typeof lang!=='undefined'&&lang==='ar')strong.textContent='مركز خدمات المرضى'}if(small){small.dataset.ar='قسم القلب بالإسكندرية';if(typeof lang!=='undefined'&&lang==='ar')small.textContent='قسم القلب بالإسكندرية'}}
...
function setup(){const box=$id('patientSearch'),btn=$id('searchBtn'),head=$id('patientRows')?.closest('table')?.querySelector('thead tr');syncDrawerBrand();document.getElementById('appLang')?.addEventListener('click',()=>setTimeout(syncDrawerBrand,0));if(!box||!btn)return;
