...
if (!document.querySelector('link[data-feature-gates-styles]')) {
  ...
  link.rel = "stylesheet";
  link.href = new URL("./feature-gates-v141.css?v=1.0.143", import.meta.url).href;
...
function addSoonBadgeToChaptersNav() {
  ...
  if (!button) return;
  const existing = button.querySelector(".feature-nav-soon");
...
function gateChapters() {
  ...
  if (route() !== "chapters" || state.flags.chapters || isOwner()) {
    if (content.dataset.featureChaptersGate) delete content.dataset.featureChaptersGate;
    return;
  }
...
function gatePriorExperience() {
  ...
  content.querySelectorAll('[class*="prior-"], [id*="prior"], [class*="prior_"], [id*="prior_"]').forEach((node) => {
    ...
    if (!node.classList.contains("feature-prior-hidden-v141")) node.classList.add("feature-prior-hidden-v141");
    foundPriorUi = true;
  });
...
function ensureAdminControl() {
  ...
  if (existing) {
    ...
    const enabled = Boolean(state.flags[key]);
    if (status) {
      if (status.textContent !== (enabled ? "Allowed" : "Not allowed")) status.textContent = enabled ? "Allowed" : "Not allowed";
...
async function refreshFlags() {
  ...
  try {
    ...
    (data || []).forEach((row) => {
      ...
      if (state.flags[row.feature_key] !== next) {
    ...
    console.warn("Could not refresh feature access", error);
  } finally {
...
setInterval(refreshFlags, 12000);
