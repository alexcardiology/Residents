...
const modal=(html)=>{const body=document.querySelector("#modalBody"),dialog=document.querySelector("#modal");if(!body||!dialog)return;body.innerHTML=html;if(!dialog.open)dialog.showModal()};
...
function targetModes(row){
  const modes=[["attended","👁","Attended"],["assisted","🤝","Performed assisted"],["solo_unguided","🩺","Performed unassisted"],["supervised","👥","Supervised"]];
...
function groupFaculty(rows){const map=new Map();rows.filter(r=>r.review_kind==="scope_verification"&&r.assessor_level==="faculty").forEach(r=>{const k=String(r.logbook_id);if(!map.has(k))map.set(k,[]);map.get(k).push(r)});return map}
function otherRowHtml(row){return `<tr><td><b>${esc(row.resident_name)}</b><small>Year ${esc(row.residency_year)}</small></td><td><div class="prior-request-stage"><span>${row.assessor_level==="professor"?"Stage 3":"Stage 1"}</span><b>${row.assessor_level==="professor"?"Professor field audit":"Senior verification"}</b><small>${esc(row.scope_name||"Prior Experience")}</small></div></td><td>${row.reconsideration_status==="requested"?'<span class="tag warning">Reconsideration requested</span>':'<span class="tag warning">Pending verification</span>'}</td><td><button class="btn small" data-prior-review-open="${esc(row.review_kind)}~${esc(row.review_id)}">${row.reconsideration_status==="requested"?"Review reconsideration":"Review evidence"}</button></td></tr>`}
function rebuildQueue(){
  const title=String(document.querySelector("#title")?.textContent||"").trim().toLowerCase();if(title!=="logbook requests")return;
  const section=document.querySelector(".prior-review-queue");if(!section)return;
  const rows=activeRows(),faculty=groupFaculty(rows),nonFaculty=rows.filter(r=>!(r.review_kind==="scope_verification"&&r.assessor_level==="faculty"));
  const wasGrouped=section.dataset.residentThreadsV127==="1";
  if(!faculty.size&&!wasGrouped)return;
  if(!faculty.size&&!nonFaculty.length){section.remove();return;}
  if(section.dataset.residentThreadsV127==="1")return;
  const body=section.querySelector("tbody");if(!body)return;
  const threadRows=[...faculty.values()].map(group=>{const first=group[0],rec=group.filter(r=>r.reconsideration_status==="requested").length;return `<tr class="prior-thread-queue-row"><td><b>${esc(first.resident_name)}</b><small>Year ${esc(first.residency_year)}</small></td><td><div class="prior-request-stage"><span>Stage 2</span><b>First-level resident thread</b><small>${group.length} manual${group.length===1?"":"s"} currently need your decision</small></div></td><td>${rec?`<span class="tag warning">${rec} reconsideration${rec===1?"":"s"}</span>`:`<span class="tag warning">${group.length} pending manual${group.length===1?"":"s"}</span>`}</td><td><button class="btn small" data-prior-resident-thread="${esc(first.logbook_id)}">Open resident thread</button></td></tr>`});
  body.innerHTML=[...threadRows,...nonFaculty.map(otherRowHtml)].join("");
...
function reconsiderButton(row){const key=`scope_verification~${row.review_id}`;return `<div class="prior-thread-reconsider"><span>Resident requested reconsideration</span><p>${esc(row.reconsideration_reason||"No reason provided")}</p><button type="button" class="btn small reclaim-button" data-prior-review-open="${esc(key)}">Review reconsideration</button></div>`}
function manualCard(row,target){
  const below=target&&!target.overall_met,status=String(row.status||"pending"),completed=["approved","rejected"].includes(status)&&row.reconsideration_status!=="requested";
...
async function openThread(logbookId){
  try{
    ...
    const first=rows[0],targetMap=new Map(targets.map(r=>[String(r.intervention_name),r])),pending=rows.filter(r=>r.status==="pending"||r.reconsideration_status==="requested").length;
...
async function submitDecision(button){
  ...
  const decision=button.dataset.threadDecision,note=String(card.querySelector("[data-thread-note]")?.value||"").trim(),below=button.dataset.belowTarget==="1",reviewId=Number(button.dataset.reviewId);
  ...
  try{
    unwrap(await sb.rpc("review_prior_experience_v1069",{p_review_kind:"scope_verification",p_review_id:reviewId,p_decision:decision,p_note:note||null}));
    const map=window.priorExperienceReviewRows;if(map?.entries){for(const [key,row] of map.entries()){if(Number(row.review_id)===reviewId&&row.review_kind==="scope_verification"){map.delete(key);break}}}
    const section=document.querySelector(".prior-review-queue");if(section)section.dataset.residentThreadsV127="";
    rebuildQueue();
    toast(`${decision==="approved"?"Approved":"Rejected"} · manual decision saved`);
    const logbookId=Number(document.querySelector(".prior-resident-thread")?.dataset.threadLogbook||0);if(logbookId)await openThread(logbookId)
  }catch(error){alert(error?.message||String(error));button.disabled=false}
}
document.addEventListener("click",async(event)=>{
  const thread=event.target.closest("[data-prior-resident-thread]");if(thread){event.preventDefault();event.stopImmediatePropagation();await openThread(thread.dataset.priorResidentThread);return}
  const decision=event.target.closest("[data-thread-decision]");if(decision){event.preventDefault();event.stopImmediatePropagation();await submitDecision(decision)}
},true);
