...
function focusHistoryThenConfirm(){
  let tries=0;
  const tick=()=>{tries++;const table=document.querySelector('.logbook-history-table-card, .printable-logbook, .logbook-history-table');if(table){table.scrollIntoView({behavior:'smooth',block:'start'});setTimeout(showSuccess,260);return;}if(tries<18)setTimeout(tick,120);else showSuccess();};
  setTimeout(tick,100);
}
sb.rpc=async function(name,args,...rest){const result=await originalRpc(name,args,...rest);if(name==='submit_logbook_entry_v1073'&&!result?.error&&!successPending){successPending=true;setTimeout(()=>{successPending=false;focusHistoryThenConfirm()},80);}return result;};
document.addEventListener('click',e=>{if(e.target.closest?.('[data-logbook214-close]')){e.preventDefault();closeSuccess();}else if(e.target.id==='logbook214Overlay'){closeSuccess();}},true);
