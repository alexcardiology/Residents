...
function enhanceRows(){
 ...
 document.querySelectorAll('.logbook-history-row').forEach(row=>{
  ...
  const suspend=document.createElement('button');suspend.type='button';suspend.className='btn small admin-logbook206-suspend';suspend.dataset.adminLogbook206Suspend=id;suspend.textContent='Suspend';suspend.style.color='#111';
  wrap.append(approve,suspend);seniorCell.append(wrap);
 });
}
...
async function openSuspension(entry){
 const seniorId=entry.senior_resident_id;if(!seniorId)return alert('No pending senior resident is assigned to this request.');
 const seniorName=entry.senior_resident_name||'Senior resident';
 document.querySelector('#adminLogbook206Overlay')?.remove();
 const rows=await history(seniorId);const active=rows.find(r=>r.active);
 const el=document.createElement('div');el.id='adminLogbook206Overlay';el.className='admin-logbook206-overlay';
 el.innerHTML=`<section class="admin-logbook206-panel"><div class="admin-logbook206-head"><div><small>ADMIN · OVERDUE SENIOR RESPONSE</small><h2>${esc(seniorName)}</h2><p>This senior resident has not responded to a junior logbook approval request within 48 hours. Suspension blocks the senior resident from recording new E-logbook activity; it does not suspend the junior applicant.</p></div><button class="admin-logbook206-close" data-admin-logbook206-close>×</button></div>
 ...
 <form class="admin-logbook206-form" data-admin-logbook206-form data-resident-id="${esc(seniorId)}"><label><b>Suspension period</b><select name="duration" required><option value="3_days">3 days</option><option value="1_week">1 week</option><option value="2_weeks">2 weeks</option><option value="1_month">1 month</option><option value="3_months">3 months</option></select></label><label><b>Reason / note</b><textarea name="reason" placeholder="Optional reason shown to the senior resident">Delayed response to a junior resident logbook request for more than 48 hours.</textarea></label><div class="actions"><button class="btn danger" type="submit">Suspend senior E-logbook recording</button>${active?'<button class="btn secondary" type="button" data-admin-logbook206-lift>End current suspension now</button>':''}</div></form>
...
async function doApprove(id,button){const entry=window.logbookEntryRows?.get?.(String(id));if(!entry)return;if(!confirm(`Approve this overdue senior-resident stage as Dr. Mohamed Alaa in place of ${entry.senior_resident_name||'the assigned senior resident'}?`))return;button.disabled=true;try{const {data,error}=await sb.rpc('owner_approve_logbook_on_behalf_v206',{p_entry_id:id});if(error)throw error;alert(`${data?.display||'Dr. Mohamed Alaa'} approved the overdue senior stage.`);location.reload()}catch(e){alert(e?.message||String(e));button.disabled=false}}
...
document.addEventListener('click',async e=>{
 ...
 const lift=e.target.closest?.('[data-admin-logbook206-lift]');if(lift){const form=lift.closest('[data-admin-logbook206-form]');if(!confirm('End this senior resident’s current E-logbook suspension now?'))return;lift.disabled=true;const {error}=await sb.rpc('owner_lift_logbook_suspension_v206',{p_resident_id:form.dataset.residentId});if(error){alert(error.message);lift.disabled=false;return}document.querySelector('#adminLogbook206Overlay')?.remove()}
},true);

document.addEventListener('submit',async e=>{const form=e.target.closest?.('[data-admin-logbook206-form]');if(!form)return;e.preventDefault();const fd=new FormData(form);const btn=form.querySelector('button[type="submit"]');btn.disabled=true;try{const {error}=await sb.rpc('owner_suspend_logbook_recording_v206',{p_resident_id:form.dataset.residentId,p_duration_code:String(fd.get('duration')),p_reason:String(fd.get('reason')||'')||null});if(error)throw error;document.querySelector('#adminLogbook206Overlay')?.remove();alert('Senior resident logbook recording suspended.')}catch(err){alert(err?.message||String(err));btn.disabled=false}},true);
