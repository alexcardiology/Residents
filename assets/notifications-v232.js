...
function nativePlatform() {
  try { return window.Capacitor?.getPlatform?.() || "android"; } catch (_) { return "android"; }
}
...
async function unregisterNativeToken() {
  ...
  if (!token) return;
  try { await sb.rpc("unregister_native_push_token_v231", { p_token: token }); } catch (_) {}
  localStorage.removeItem(NATIVE_TOKEN_KEY);
  nativeToken = "";
  // Keep PUSH_PREFERENCE_KEY: if OS permission is still granted, next sign-in silently re-registers this device.
}
...
async function ensureNativeSubscription(askPermission = false) {
  ...
  try {
    ...
    if (askPermission && permission?.receive === "prompt") permission = await plugin.requestPermissions();
...
function installButton() {
  ...
  btn.addEventListener("click", async () => {
    ...
    if (capacitorShell() || !fullySupportedWeb()) return alert(unsupportedMessage());
...
async function repairPushSubscription() {
  try {
    ...
    if (nativePushPlugin()) {
      ...
      if (ok) localStorage.setItem(PUSH_PREFERENCE_KEY, "1");
      return ok;
    }
    ...
    console.warn("Silent push subscription repair failed", error);
    return false;
  }
}
...
async function boot() {
  ...
  if (nativePushPlugin()) {
    ...
    await createNativeChannel();
    if (nativeToken) {
      try { await saveNativeToken(nativeToken); } catch (_) {}
    }
  }
...
document.addEventListener("click", (event) => {
  if (event.target.closest?.("#logout") && nativePushPlugin()) void unregisterNativeToken();
}, true);
sb.auth.onAuthStateChange((_event, session) => { if (session) setTimeout(boot, 100); });
boot();


// Once notification permission has been granted, silently keep the device subscription healthy.
// Browsers do not allow a site to override an OS/browser permission that the user later disables.
window.addEventListener("focus", () => { void repairPushSubscription(); });
window.addEventListener("online", () => { void repairPushSubscription(); });
window.addEventListener("pageshow", () => { void repairPushSubscription(); });
document.addEventListener("visibilitychange", () => {
  if (document.visibilityState === "visible") void repairPushSubscription();
});
