...
function restoreBanner() {
  ...
  /* Older versions of this helper created their own Coming Soon card. Remove
     those legacy duplicates; feature-gates-v141.js is now the single owner of
     the visible Prior Experience Coming Soon card. */
  document.querySelectorAll('[data-prior-banner-gate-soon="1"]').forEach((node) => node.remove());
}

function removeLegacyDuplicateNotices() {
  document.querySelectorAll('[data-prior-banner-gate-soon="1"]').forEach((node) => node.remove());
}

function paint() {
  if (!ready) return;
  removeLegacyDuplicateNotices();
