...
async function boot() {
  ...
  if (nativePushPlugin()) {
    ...
    if (permission === "granted") {
      try { await ensureNativeSubscription(false); } catch (error) { console.warn("Native push registration failed", error); }
    }
    return;
  }
  if (fullySupportedWeb() && Notification.permission === "granted") {
    try { await ensureWebSubscription(false); } catch (error) { console.warn("Web push registration failed", error); }
  }
}
