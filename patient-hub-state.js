...
// Prevent any Dashboard / expanded-drawer flash while an existing session is restoring.
...
document.addEventListener('click',e=>{
  ...
  if(t){
    ...
    setTimeout(()=>{const el=document.getElementById(id);if(el)write({[id+'Collapsed']:el.classList.contains('hidden')})},0);
  }
},true);
...
function restoreDrawer(st){
  ['cath','echo'].forEach(id=>{
    const el=document.getElementById(id),key=id+'Collapsed';
    if(el&&typeof st[key]==='boolean')el.classList.toggle('hidden',st[key]);
  });
}
function reveal(){if(app)app.style.visibility='visible'}
async function restore(){
  ...
  if(st.page==='service'&&st.service){
    ...
    if(typeof loadService==='function')await loadService();
    restoreDrawer(st);
    reveal();
    return;
  }
  if(st.page&&st.page!=='dashboard'){
    if(document.getElementById(st.page)){show(st.page);restoreDrawer(st);reveal();return}
    if(st.page==='adminCath'){
      let tries=0;const a=setInterval(()=>{tries++;if(document.getElementById('adminCath')){clearInterval(a);show('adminCath');restoreDrawer(st);reveal()}else if(tries>30){clearInterval(a);reveal()}},50);return;
    }
  }
  restoreDrawer(st);reveal();
}

let n=0;const t=setInterval(()=>{
  n++;
  if(typeof profile!=='undefined'&&profile&&app&&!app.classList.contains('hidden')){
    clearInterval(t);restore();
  }else if(n>120){clearInterval(t);reveal()}
},50);

window.addEventListener('beforeunload',()=>{
  const active=document.querySelector('.page.on')?.id||'dashboard';
  const st={page:active,service:active==='service'?currentServiceCode():null,date:document.getElementById('serviceDate')?.value||'',status:document.getElementById('serviceStatus')?.value||'',rangeFrom:document.getElementById('rangeFrom')?.value||'',rangeTo:document.getElementById('rangeTo')?.value||''};
  ['cath','echo'].forEach(id=>{const el=document.getElementById(id);if(el)st[id+'Collapsed']=el.classList.contains('hidden')});write(st);
});
})();