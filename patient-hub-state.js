...
function syncParentOpenState(){
 ['cath','echo'].forEach(id=>{
   ...
   const parent=document.querySelector(`.nav [data-toggle="${id}"]`);
   if(parent&&el)parent.classList.toggle('open-section',!el.classList.contains('hidden'));
 });
}
function restoreDrawer(st){['cath','echo'].forEach(id=>{const el=document.getElementById(id),k=id+'Collapsed';if(el&&typeof st[k]==='boolean')el.classList.toggle('hidden',st[k])});syncParentOpenState()}
...
installDrawerLabels();syncParentOpenState();
...
document.addEventListener('click',e=>{const p=e.target.closest('[data-page]');if(p)write({page:p.dataset.page,service:null});const s=e.target.closest('[data-service]');if(s)write({page:'service',service:s.dataset.service});const t=e.target.closest('[data-toggle]');if(t){const id=t.dataset.toggle;setTimeout(()=>{const el=document.getElementById(id);if(el)write({[id+'Collapsed']:el.classList.contains('hidden')});syncParentOpenState()},0)}},true);
...
const bulk=document.createElement('script');bulk.src='./patient-hub-list-booking.js?v=20260819-3';document.body.appendChild(bulk);
})();