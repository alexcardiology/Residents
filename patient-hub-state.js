...
function installDrawerLabels(){
 const labels={
  cath_miri:['Miri','الميري'],cath_smouha:['Smouha','سموحة'],echo_miri:['Miri','الميري'],echo_smouha:['Smouha','سموحة'],tee:['TEE','إيكو بالمنظار'],dse:['DSE','إيكو بالمجهود الدوائي'],holter:['Holter','رسم قلب ممتد هولتر'],exercise_ecg:['Exercise ECG','رسم قلب بالمجهود']
 };
...
async function restoreState(){const st=read();restoreDrawer(st);try{if(st.page==='service'&&st.service&&typeof openService==='function'){await openService(st.service);const d=document.getElementById('serviceDate'),s=document.getElementById('serviceStatus'),rf=document.getElementById('rangeFrom'),rt=document.getElementById('rangeTo');if(d&&st.date!==undefined)d.value=st.date||'';if(s&&st.status!==undefined)s.value=st.status||'';if(rf&&st.rangeFrom!==undefined)rf.value=st.rangeFrom||'';if(rt&&st.rangeTo!==undefined)rt.value=st.rangeTo||'';if(typeof loadService==='function')await loadService();restoreDrawer(st);return}if(st.page&&st.page!=='dashboard'&&document.getElementById(st.page)&&typeof show==='function'){show(st.page);restoreDrawer(st)}}catch(e){console.error('PSH state restore failed',e)}}
